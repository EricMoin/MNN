...
void Talker::processWavChunk(WavChunk& chunk) {
    ...
    double vocoder_time_ms = std::chrono::duration<double, std::milli>(t_voc_end - t_voc_start).count();
    {
        ...
        PERF_PRINT("stage=v2 thread=vocoder phase=vocoder_end chunk_id=%d vocoder_time_ms=%.3f waveform_dims=%d,%d,%d,%d",
                   chunk_id, vocoder_time_ms,
...
void Talker::generate() {
    ...
    PERF_PRINT("stage=v2 phase=generate_start timestamp_ms=%.3f", ts_ms);
#endif
    ...
    PERF_PRINT("stage=v2 phase=generate_end total_time_ms=%.3f total_chunks=%d", total_time_ms, total_chunks);
#endif
}
